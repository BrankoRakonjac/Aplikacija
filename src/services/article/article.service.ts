...
import { ArticleSearchDeatureComponentDto } from "src/dtos/article/article.search.feature.component";
...
export class ArticleService extends TypeOrmCrudService<Article>{
    ...
    async editFullArticle(articleId: number , data: EditArticleDto ):Promise<Article | ApiResponse>{
        ...
        const savedArtice = await  this.article.save(existingArticle);
        if(!savedArtice){
        ...
        if(newPriceString !== lastPriceString){
           ...
           if(!savedarticlePrice){
               return new ApiResponse('error' , -5003, 'Could not save new article prices')
           }
